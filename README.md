...
| `AL_EXTRA_CODE_ANALYZERS` | No | — | Semicolon-separated analyzer DLLs to load **on top of** `al.codeAnalyzers` from `.vscode/settings.json`. Supports `${analyzerFolder}`, `${workspaceFolder}`, `${CodeCop}`, `${AppSourceCop}`, etc. placeholders. |
| `AL_EXTRA_RULESETS` | No | — | Semicolon-separated `*.ruleset.json` paths to apply **in addition to** `al.ruleSetPath`. The bridge merges multiple rulesets into a composite file under the OS temp directory. |
